...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		Logger.getLogger(Main.class.getName()).log(Level.CONFIG, "launch4j.exedir={0}",
		...
		if (configFile.exists()) {
			...
			Logger.getLogger(Main.class.getName()).log(Level.CONFIG, "No config file found: <{0}>",
					configFile.getPath());
		}
...
//                        }
	 ...
	 * size. This is necessary for two reasons:
	 * <ul>
	 * <li>The Nimbus LaF is initialized dynamically, so setting it in UIManager and
	 * changing the defaults wont work (instance has not yet been created, so the
	 * changes wont work)</li>
	 * <li>The package of the Nimbus LaF changed from Java 1.6 to 1.7. Simply
	 * subclassing NimusLookAndFeel and overriding getDefaults() throws an
	 * ClassLoaderException on some Java versions.</li>
	 * </ul>
	 *
	 * @param className
	 * @param fontName
...
//    }
	...
	/**
	 * Parses a command line string that comes from a file or from stdin. This used
	 * to be a String.split(" "), but we have to support text qualifiers.
	 *
	...
	public void run() {
		...
		try {
			...
			while (!isInterrupted()) {
				...
				solver.solve();
				// System.out.println("result: " + clonedSudoku.isSolved() + "/" +
				// clonedSudoku.getLevel().getName());
				if (level != null) {
						...
						if (type.equals(puzzleTypes[j].type.type)) {
							...
							if (type.isSingle()) {
								...
								if (puzzleTypes[j].typeSeen) {
									...
									if (puzzleTypes[j].isPuzzleMode1 == 3 || puzzleTypes[j].isPuzzleMode1 == -1) {
	...
	private void printStatistic(PrintWriter out, StepStatistic[] stat, boolean total) throws IOException {
		...
		for (int i = 0; i < stat.length; i++) {
			if (stat[i].anzSteps > 0) {
				if (out != null) {
					out.printf("      %8d - %8d/%8d: %s", stat[i].anzSteps, stat[i].anzSet, stat[i].anzCandDel,
							stat[i].type.getStepName());
//                    out.write("      " + stat[i].type.getStepName() + ": " + stat[i].anzSteps + " - " + stat[i].anzSet + "/" + stat[i].anzCandDel);
					out.println();
					if (bruteForceTest) {
						out.printf("      Invalid: %3d - %3d/%3d", stat[i].anzInvalidSteps, stat[i].anzInvalidSet,
								stat[i].anzInvalidCandDel);
...
//                    System.out.println("      " + stat[i].type.getStepName() + ": " + stat[i].anzSteps + " - " + stat[i].anzSet + "/" + stat[i].anzCandDel);
					System.out.printf("      %8d - %8d/%8d: %s", stat[i].anzSteps, stat[i].anzSet, stat[i].anzCandDel,
							stat[i].type.getStepName());
					System.out.println();
					if (bruteForceTest) {
//                        System.out.println("        Invalid: " + stat[i].anzInvalidSteps + " - " + stat[i].anzInvalidSet + "/" + stat[i].anzInvalidCandDel);
						System.out.printf("        Invalid: %3d - %3d/%3d", stat[i].anzInvalidSteps,
								stat[i].anzInvalidSet, stat[i].anzInvalidCandDel);
						System.out.println();
					}
				}
			}
...
//                    System.out.println("solved!");
				...
				if (printSolution || bruteForceTest) {
					...
					if (sudoku.isSolved()) {
						...
						// solvedSudoku.getSudoku(ClipboardMode.PM_GRID));
						generator.validSolution(solvedSudoku);
						// System.out.println("SolvedSudoku2: " +
						// solvedSudoku.getSudoku(ClipboardMode.PM_GRID));
					...
					for (int i = 0; i < steps.size(); i++) {
						if (outputGrid || bruteForceTest) {
							if (types != null && clipboardMode != null && types.contains(steps.get(i).getType())
								...
								int end = clipboardMode == ClipboardMode.PM_GRID_WITH_STEP ? gridLines.length - 2
										: gridLines.length;
								for (int j = 0; j < end; j++) {
									if (outFile != null) {
	...
	public static void parseTypeStr(List<StepType> stepList, String inputStr) {
		...
		if (type == null) {
			if (inputStr.equals("all")) {
				...
				for (SolutionType tmpType : SolutionType.values()) {
					if (!tmpType.isSingle() && !tmpType.isSSTS() && !tmpType.equals(SolutionType.TWO_STRING_KITE)
							&& !tmpType.equals(SolutionType.SKYSCRAPER) && !tmpType.equals(SolutionType.BUG_PLUS_1)
							&& !tmpType.equals(SolutionType.EMPTY_RECTANGLE) && !tmpType.equals(SolutionType.W_WING)
							&& !tmpType.equals(SolutionType.UNIQUENESS_1) && !tmpType.equals(SolutionType.XYZ_WING)
...
//                    tmpStep.compAnz = step.compAnz;
		...
		if (step.isRemove) {
			if (!found) {
				System.out.println("Could not remove step " + step.type.getArgName() + ":" + step.puzzleType
						+ ": was not set yet.");
			}
		} else {
			stepList.add(step);
