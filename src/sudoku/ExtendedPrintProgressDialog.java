...
/**
 * This class handles extended printing (printing with different layouts). It is
...
public class ExtendedPrintProgressDialog extends javax.swing.JDialog implements Runnable, Printable {
	/**
	 * Standard factor for calculating gaps: half a cell (two sudokus means 18 cells
	 * plus the gap)
	 */
	...
	/**
	 * An array with the puzzles that have to be printed. Synchronized with
	 * {@link #candidates}
	 */
	private Sudoku2[] sudokus;
	/**
	 * An array with "print candidates" options for every puzzle. Synchronized with
	 * {@link #sudokus}.
	 */
	...
	/**
	 * A flag to indicate whether all properties for the current job have been
	 * initalized.
	 */
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public ExtendedPrintProgressDialog(java.awt.Frame parent, boolean modal, JTextField[] numberTextFields,
			JComboBox[] levelComboBoxes, JComboBox[] modeComboBoxes, JCheckBox[] candCheckBoxes, int layout,
	...
	private void initComponents() {
		...
		cancelButton.setMnemonic(java.util.ResourceBundle.getBundle("intl/ExtendedPrintProgressDialog")
				.getString("ExtendedPrintProgressDialog.cancelButton.mnemonic").charAt(0));
		...
		layout.setHorizontalGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING).addGroup(layout
				...
				.addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
						.addGroup(layout.createSequentialGroup().addGap(10, 10, 10).addComponent(printProgressBar,
		...
		layout.setVerticalGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGroup(layout.createSequentialGroup().addGap(26, 26, 26)
						.addComponent(printProgressBar, javax.swing.GroupLayout.PREFERRED_SIZE,
								javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
						.addGap(18, 18, 18)
						.addComponent(jPanel1, javax.swing.GroupLayout.PREFERRED_SIZE, 33,
	...
	private void cancelButtonActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_cancelButtonActionPerformed
		...
		try {
			...
			Logger.getLogger(getClass().getName()).log(Level.SEVERE, "Interrupted while waiting for generation thread",
					ex);
		}
		setVisible(false);
	...
	/**
	 * The background thread: creates the puzzles and prints them. Can be
...
//                        System.out.println("2 job: " + job + "/ service: " + job.getPrintService());
	...
	private boolean createSudokus() {
		...
		int index = 0;
		for (int i = 0; i < numberTextFields.length; i++) {
			DifficultyLevel actDiffLevel = Options.getInstance()
					.getDifficultyLevel(levelComboBoxes[i].getSelectedIndex() + 1);
	...
	/**
	 * Creates a new Sudoku with a given{@link DifficultyLevel} and a given
	...
	private Sudoku2 getSudoku(DifficultyLevel level, GameMode mode) {
		...
		if (preGenSudoku == null) {
			...
			solver.solve(level, solvedSudoku, true, null, false, Options.getInstance().solverSteps,
...
//        printG2.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIALIAS_ON);
		...
		if (!initialized) {
			...
			if (!printBooklet) {
				...
				// insets (left inset - puzzle - right inset - left inset - puzzle - right
				// inset)
			...
			// actual imageSize
			imagePrintSize = (borderWidth < (borderHeight - footerHeight)) ? borderWidth
	...
	private void printSudoku(Graphics2D g2, int index, int position, double scale) {
		...
		if (printRating && sudoku != null && sudoku.getLevel() != null) {
			...
			g2.drawString(title, startX + imagePrintSize / 2 - textWidth / 2,
					(int) (startY + imagePrintSize + textHeight * 1.5));
		}
	}
	...
	/**
	 * Updates the progress bar. {@link #percentage} MUST be accessed via the getter
	 * to provide synchronization. {@link #numberOfPages} is only read and never
	 * updated during the process,so synchronization is not necessary.
	 */
	private void setProgress() {
		int value = getPercentage();
		if (value >= 0) {
			setTitle(java.util.ResourceBundle.getBundle("intl/ExtendedPrintProgressDialog")
					.getString("ExtendedPrintProgressDialog.title"));
			printProgressBar.setValue(value);
			printProgressBar.setString(value + " %");
		} else {
			value = -value;
			setTitle(java.util.ResourceBundle.getBundle("intl/ExtendedPrintProgressDialog")
					.getString("ExtendedPrintProgressDialog.title2"));
	...
	private void showMessageDialog() {
		int ret = JOptionPane.showConfirmDialog(null,
				java.util.ResourceBundle.getBundle("intl/ExtendedPrintProgressDialog")
						.getString("ExtendedPrintProgressDialog.flipPaper"),
				java.util.ResourceBundle.getBundle("intl/ExtendedPrintProgressDialog")
						.getString("ExtendedPrintProgressDialog.flipPaperTitle"),
