...
//        return getForcingChainString(chain.getChain(), chain.getStart(), chain.getEnd(), false);
	...
	public StringBuffer getForcingChainString(int[] chain, int start, int end, boolean weakLinks) {
		...
		for (int i = start + 1; i <= end - 1; i++) {
			...
			if (!weakLinks && !Chain.isSStrong(chain[i])
				...
				// group nodes, als etc. nicht überspringen (sonst kennt man sich gar nicht
				// mehr aus
	...
	public String getCandidateString(boolean library, boolean statistics) {
		...
		if (library) {
			...
			if (statistics) {
				...
				return candBuff.toString() + " (" + getAnzCandidatesToDelete() + "):" + delPos.toString() + " ("
						+ tmpStepName + ")";
			}
		}
	}
	...
	public int getAnzSet() {
		...
		if (type == SolutionType.FORCING_CHAIN || type == SolutionType.FORCING_CHAIN_CONTRADICTION
				|| type == SolutionType.FORCING_CHAIN_VERITY || type == SolutionType.FORCING_NET
				|| type == SolutionType.FORCING_NET_CONTRADICTION || type == SolutionType.FORCING_NET_VERITY) {
			if (indices.size() > 0) {
				return 1;
			}
		}
	...
	public String toString(int art) {
		...
		case LOCKED_PAIR:
			...
			if (art >= 1) {
				tmp.append(": ");
				if (type == SolutionType.HIDDEN_PAIR || type == SolutionType.NAKED_PAIR
						|| type == SolutionType.LOCKED_PAIR) {
					tmp.append(values.get(0));
					tmp.append(",");
					tmp.append(values.get(1));
				} else if (type == SolutionType.HIDDEN_TRIPLE || type == SolutionType.NAKED_TRIPLE
...
//    }
	...
	/**
	 * Prüft, ob Index index in einem AlsInSolutionStep enthalten ist. Wenn ja,
	 * wird der index in alses zurückgegeben, sonst -1;
	 *
	 * Doesnt work: if a step has more than one chain, a cell can be part of more
	 * than one ALS; index has to be checked against a chain (only when a certain
	 * chain is requested, which means if chainIndex != -1)!
	 *
	 * @param index
	 * @param chainIndex
	 * @return
	 ...
	 * if they have not the same SolutionType Exception: both steps are fish
...
//        }
	...
	public boolean isSingle(SolutionType type) {
		return (type == SolutionType.FULL_HOUSE || type == SolutionType.HIDDEN_SINGLE
				|| type == SolutionType.NAKED_SINGLE || type == SolutionType.TEMPLATE_SET);
	}
	 ...
	 * Sortierreihenfolge:
	 *
	 * - Die Steps mit den meisten zu löschenden Kandidaten zuerst - Dann nach
	 * betroffenen Kandidaten - dann nach Äquvalenz (wenn nicht äquivalent, nach
	 * Summe der Indexe der zu löschenden Kandidaten - dann nach betroffenen
	 * Kandidaten und Fins - innerhalb der gleichen Steps nach type
	 */
	@Override
...
//        System.out.println("c: " + getType()+ "/"+ getSubType()+" - " + o.getType() + "/" + o.getSubType());
		if (type.isKrakenFish() && o.getType().isKrakenFish()) {
			// if (type == SolutionType.KRAKEN_FISH && o.getType() ==
...
//        }
	...
	/**
	 * @param potentialCannibalisticEliminations the
	 *                                           potentialCannibalisticEliminations
	 *                                           to set
	 */
