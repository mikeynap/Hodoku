...
/**
 * A chain consists of links, every link can be weak or strong, it can be a
 * candidat in a cell, a group node, an ALS, AUR...<br>
 * <br>
 *
 ...
 * <br>
 *
 * An entry can be negativ to indicate that it is a starting point for a branch
 * in a net.<br>
 * <br>
 *
 * The class contains a set of static methods to do the bit manipulations for a
 * single node and a corresponding set of methods that do the same for entries
 * in {@link #chain}, that delegate to the static methods.
 *
 * @author hobiwan
 */
public class Chain implements Cloneable {
	...
	/**
	 * Every index consists of 7 bits (max: 128). To isolate an index using this
	 * mask the entry has to be shifted by the appropriate INDEXn_OFFSET.
	 */
	...
	/**
	 * Number of bits that the entry has to be right shifted to get the index1 to
	 * the right.
	 */
	...
	/**
	 * Number of bits that the entry has to be right shifted to get the index2 to
	 * the right.
	 */
	...
	/**
	 * Number of bits that the entry has to be right shifted to get the index3 to
	 * the right.
	 */
	...
	private int end;
	/**
	 * The length of the chain. For normal nodes it equals
	 * <code>(end - start + 1)</code>, for complex chains a penalty for the complex
	 * nodes is added (see {@link #calculateLength(java.util.List)}). A value of
	 * <code>-1</code> means, that the length has not been calculated yet.
	 */
	private int length;
...
//        newChain.chain = chain.clone();
	...
	/**
	 * Calculates the length of the chain: If the chain contains complex nodes, a
	 ...
	 * @param cellIndex3 Index of the third cell in a group node, upper 7 bits of an
	 *                   ALS index or -1 if unused
	 * @param candidate  The exit candidate for the link
	 * @param isStrong   The type of the link (weak or strong)
	 * @param nodeType   The node type ({@link #NORMAL_NODE}, {@link #GROUP_NODE} or
	 *                   {@link #ALS_NODE}).
	 * @return
	 */
	public static int makeSEntry(int cellIndex1, int cellIndex2, int cellIndex3, int candidate, boolean isStrong,
			int nodeType) {
...
//        System.out.println("   " + toString(entry));
	...
	/**
	 * Replace the index of the als used in the link. Is needed when chains with ALS
	 ...
	 * @param alses     A list with all alses for that chain (only valid for ALS
	 *                  nodes)
	...
	public static void getSNodeBuddies(int entry, int candidate, List<Als> alses, SudokuSetBase set) {
		if (getSNodeType(entry) == NORMAL_NODE) {
			...
			set.clear();
			Logger.getLogger(Chain.class.getName()).log(Level.SEVERE,
					"getSNodeBuddies() gesamt: invalid node type ({0})", getSNodeType(entry));
		}
	}
...
//    }
	...
	public static String toString(int entry) {
		...
		if (getSNodeType(entry) == ALS_NODE) {
			return sign + TYPE_NAMES[getSNodeType(entry)] + "/" + getSAlsIndex(entry) + "/" + getSCellIndex(entry) + "/"
					+ isSStrong(entry) + "/" + getSCandidate(entry);
		} else {
			return sign + TYPE_NAMES[getSNodeType(entry)] + "/" + getSCellIndex3(entry) + "/" + getSCellIndex2(entry)
