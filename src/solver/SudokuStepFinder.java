...
public class SudokuStepFinder {
	 ...
	 * @return
	 */
	public List<SolutionStep> getAllFishes(Sudoku2 newSudoku, int minSize, int maxSize, int maxFins, int maxEndoFins,
			FindAllStepsProgressDialog dlg, int forCandidate, int type) {
		initialize();
		Sudoku2 oldSudoku = getSudoku();
		setSudoku(newSudoku);
		List<SolutionStep> steps = fishSolver.getAllFishes(minSize, maxSize, maxFins, maxEndoFins, dlg, forCandidate,
				type);
		setSudoku(oldSudoku);
		return steps;
	}
	 ...
	 * @return
	 */
	public List<SolutionStep> getAllKrakenFishes(Sudoku2 newSudoku, int minSize, int maxSize, int maxFins,
			int maxEndoFins, FindAllStepsProgressDialog dlg, int forCandidate, int type) {
		initialize();
		Sudoku2 oldSudoku = getSudoku();
		setSudoku(newSudoku);
		List<SolutionStep> steps = fishSolver.getAllKrakenFishes(minSize, maxSize, maxFins, maxEndoFins, dlg,
				forCandidate, type);
		setSudoku(oldSudoku);
		return steps;
	}
...
//        }
		...
		if ((initLists && templatesListDirty) || (!initLists && templatesDirty)) {
			...
			SudokuSetBase[] forbiddenPositions = new SudokuSetBase[10]; // eine 1 an jeder Position, an der Wert nicht
																		// mehr sein darf
...
//        SudokuSetBase temp = new SudokuSetBase();
	 ...
	 * {@link #collectAllAlsesForHouse(int[][], sudoku.Sudoku2, java.util.List, boolean)}.<br>
	 * <br>
	...
	public String getAlsStatistics() {
		return "Statistic for getAls(): number of calls: " + anzAlsCalls + ", total time: " + (alsNanos / 1000)
				+ "us, average: " + (alsNanos / anzAlsCalls / 1000) + "us\r\n" + "    anz: " + anzAls + "/"
				+ (anzAls / anzAlsCalls) + ", double: " + doubleAls + "/" + (doubleAls / anzAlsCalls) + " res: "
				+ (anzAls - doubleAls) + "/" + ((anzAls - doubleAls) / anzAlsCalls);
	}
	...
	public List<RestrictedCommon> getRestrictedCommons(List<Als> alses, boolean allowOverlap) {
		if (lastRcStepNumber != stepNumber || lastRcAllowOverlap != allowOverlap || lastRcAlsList != alses
	...
	private List<RestrictedCommon> doGetRestrictedCommons(List<Als> alses, boolean withOverlap) {
		...
		for (int i = 0; i < alses.size(); i++) {
			...
			for (int j = start; j < alses.size(); j++) {
				...
				if (rcAnz > 0) {
					// if (DEBUG) System.out.println(newRC + ": " + rcAnz + " RCs for ALS " +
					// SolutionStep.getAls(als1) + "/" + SolutionStep.getAls(als2));
				}
			}
