...
//                        }
					...
					if (type == REMOTE_PAIR) {
						...
						startCellSet2M1 = Sudoku2.buddiesM1[startIndex]
								& finder.getCandidates()[startCandidate2].getMask1();
						startCellSet2M2 = Sudoku2.buddiesM2[startIndex]
								& finder.getCandidates()[startCandidate2].getMask2();
					}
					// now do it
					getChain(entry, type);
				}
		...
		switch (type) {
		case TURBOT_FISH:
			chainTNanos += System.nanoTime() - nanos;
			anzT++;
			break;
		case REMOTE_PAIR:
			chainRNanos += System.nanoTime() - nanos;
			anzR++;
			break;
		case X_CHAIN:
			chainXNanos += System.nanoTime() - nanos;
			anzX++;
			break;
		case XY_CHAIN:
			chainYNanos += System.nanoTime() - nanos;
			anzY++;
			break;
		}
	}
...
//        }
	...
	private void checkNiceLoop(int lastLink, int chainIndex) {
		...
		if (!firstLinkStrong && !lastLinkStrong && startCandidate == endCandidate) {
		...
		} else if ((!firstLinkStrong && !lastLinkStrong && sudoku.getAnzCandidates(startIndex) == 2
			...
			for (int i = 1; i <= chainIndex; i++) {
				...
				// != (chain[i] / 10) % 100) {
				if (Chain.isSStrong(chain[i]) && i <= chainIndex - 2
						&& Chain.getSCellIndex(chain[i - 1]) != Chain.getSCellIndex(chain[i])) {
					...
					// 100) {
					if (!Chain.isSStrong(chain[i + 1])
							&& Chain.getSCellIndex(chain[i]) == Chain.getSCellIndex(chain[i + 1])
							&& Chain.isSStrong(chain[i + 2])
							&& Chain.getSCellIndex(chain[i + 1]) != Chain.getSCellIndex(chain[i + 2])) {
...
//            }
	...
	private void getAllLinks(int type) {
		// caching
		if (turbotOrXSeen && (type == TURBOT_FISH || type == X_CHAIN) && (lastStepNumber) == finder.getStepNumber()) {
...
//        System.out.println("getAllLinks: " + index);
		switch (type) {
		case TURBOT_FISH:
			linkTNanos += System.nanoTime() - nanos;
			break;
		case REMOTE_PAIR:
			linkRNanos += System.nanoTime() - nanos;
			break;
		case X_CHAIN:
			linkXNanos += System.nanoTime() - nanos;
			break;
		case XY_CHAIN:
			linkYNanos += System.nanoTime() - nanos;
			break;
		}
	}
	...
	protected void printStatistics() {
		...
		System.out.printf("Turbot:   %6d/%6.2fus/%6.2fus/%6.2fus\r\n", anzT, (chainTNanos / danzT),
				(linkTNanos / danzT), ((chainTNanos - linkTNanos) / danzT));
		System.out.printf("RP:       %6d/%6.2fus/%6.2fus/%6.2fus\r\n", anzR, (chainRNanos / danzR),
				(linkRNanos / danzR), ((chainRNanos - linkRNanos) / danzR));
		System.out.printf("X-Chain:  %6d/%6.2fus/%6.2fus/%6.2fus\r\n", anzX, (chainXNanos / danzX),
				(linkXNanos / danzX), ((chainXNanos - linkXNanos) / danzX));
		System.out.printf("XY-Chain: %6d/%6.2fus/%6.2fus/%6.2fus\r\n", anzY, (chainYNanos / danzY),
				(linkYNanos / danzY), ((chainYNanos - linkYNanos) / danzY));
	}
