...
/**
 * The format of an entry is described in class {@link Chain}.<br>
 * <br>
 *
 * An instance of <code>TableEntry</code> holds all possible outcomes for one
 * single premise. The premise itself is not part of the
 * <code>TableEntry</code>. The {@link TablingSolver} holds two arrays of
 * <code>TableEntries</code>. The index of the entry decides cell and candidate
 * (<code>cell * 100 + candidate</code>), one table is for "candidate is deleted
 * from cell" and one is for "cell is set to that number".<br>
 * <br>
 *
 * A <code>TableEntry</code> consists mainly of two synchronized arrays:
 * {@link #entries} contains the possible conclusions, {@link #retIndices}
 * contains up to five indices of {@link #entries}, that have to be true to
 * reach that conclusion (used to reconstruct the chain from the table). If an
 * entry has more than one return index, the result has to be a net instead of a
 * chain.<br>
 * <br>
 *
 * Two arrays of sets, {@link #onSets} and {@link #offSets}, hold bitmaps that
 * summarize all possible conclusions for the premise. A set bit in
 * <code>onSets[cand]</code> means that that cell can be set to a value of
 * <code>cand</code>, a set bit in <code>offSets[cand]</code>, that
 * <code>cand</code> can be eliminated from that cell. The sets are used to
 * check for possible outcomes.<br>
 * <br>
 *
 ...
 * Please note:
 * <ul>
 * <li>"distance" is used in finding the shortest possible chain (number of
 * links used to get to the current entry)</li>
 * <li>if more than one index is present, index 1 is always the largest
 * numerical value (should make the shortest chain the "main" chain in
 * networks)</li>
 * <li>if "expanded" (61) is set, the index1 is an index in the table from where
 * the entry was expanded. Which table this is depends on the flags "onTable"
 * and "extended":
 * <ul>
 ...
 * <li>onTable 1 extended 1: invalid</li>
 * </ul>
 * </ul>
 *
 * @author hobiwan
...
public class TableEntry {
	/**
	 * Used by {@link #makeSRetIndex(int, int, int, int, int)}; access must be
	 * synchronized!
	 */
//    private static long[] rIndices = new long[5];
	...
	/**
	 * Bitmap indicating that the entry comes from
...
//    private static final long RAW_ENTRY      = 0x1fffffffffffffffL;
	...
	int index = 0;
	/**
	 * The actual table, holding all resulting links. Synchronized with
	 * {@link #retIndices}.
	 */
	int[] entries = new int[Options.getInstance().getMaxTableEntryLength()];
	/**
	 * Contains up to 5 reverse indices plus the distance of the entry to the root
	 * assumption. Synchronized with {@link #entries}.
	 */
	long[] retIndices = new long[Options.getInstance().getMaxTableEntryLength()];
	/**
	 * Array of sets holding all cells for every candidate that can be set as a
	 * result of the assumption.
	 */
	SudokuSet[] onSets = new SudokuSet[10];
	/**
	 * Array of sets holding all cells for every candidate that can be deleted as a
	 * result of the assumption.
	 */
	SudokuSet[] offSets = new SudokuSet[10];
	/**
	 * Reverse lookup cache: hold the index in {@link #entries} for every entry.
	 * Used when constructing the chain from the result and when expanding tables.
	 */
	...
	void addEntry(int cellIndex1, int alsIndex, int nodeType, int cand, boolean set, int penalty) {
		addEntry(cellIndex1, Chain.getSLowerAlsIndex(alsIndex), Chain.getSHigherAlsIndex(alsIndex), nodeType, cand, set,
				0, 0, 0, 0, 0, penalty);
	}
	 ...
	 *                   {@link Chain#ALS_NODE}.
	 * @param cellIndex2 -1 for {@link Chain#NORMAL_NODE}, the index of the second
	 *                   cell for {@link Chain#GROUP_NODE}; the lower 7 bits of the
	 *                   ALS index for {@link Chain#ALS_NODE}.
	 * @param cellIndex3 -1 for {@link Chain#NORMAL_NODE}; the index of the third
	 *                   cell or -1, if no third cell exists for
	 *                   {@link Chain#GROUP_NODE}; the higher 7 bits of the ALS
	 *                   index for {@link Chain#ALS_NODE}.
	 * @param nodeType   {@link Chain#NORMAL_NODE}, {@link Chain#GROUP_NODE} or
	 *                   {@link Chain#ALS_NODE}.
	 * @param cand       The candidate for the link or the entry candidate for an
	 *                   ALS link.
	 * @param set        <code>true</code> if the cell can be set to
	 *                   <code>cand</code>, <code>false</code>, if <code>cand</code>
	 *                   can be eliminated from the cell.
	 * @param ri1        The first reverse index or 0 for entries that depend
	 *                   directly on the initial assumption. For entries that have
	 *                   been expanded from another table the index of that table.
	 * @param ri2        The second reverse index for a net or 0 if no reverse index
	 *                   is present.
	 * @param ri3        The third reverse index for a net or 0 if no reverse index
	 *                   is present.
	 * @param ri4        The fourth reverse index for a net or 0 if no reverse index
	 *                   is present.
	 * @param ri5        The fifth reverse index for a net or 0 if no reverse index
	 *                   is present.
	 * @param penalty    A possible penalty for complex ALS nodes.
	 */
	void addEntry(int cellIndex1, int cellIndex2, int cellIndex3, int nodeType, int cand, boolean set, int ri1, int ri2,
			int ri3, int ri4, int ri5, int penalty) {
		...
		// check only for single cells -> group nodes, ALS etc. can not be start or end
		// point
	...
	int getEntryIndex(int cellIndex, boolean set, int cand) {
		...
		if (ret == null) {
			// Logger.getLogger(getClass().getName()).log(Level.SEVERE, "entry not found: "
			// + cellIndex + ", " + cand + ", " + set);
			return 0;
		} else {
			return ret.intValue();
...
//        return indices.get(Chain.makeSEntry(cellIndex, cand, set));
	...
	/**
	 * Constructs the data for {@link #retIndices}: Every entry contains up to 5
	 * reverse indices (indices of entries on which the entry depends). For every
	 * index 10 bits are reserved, which makes the largest value for every index
	 * 1023. The first index has 12 bits, allowing for a maximum of 4095.<br>
	 * <br>
	 *
...
//                (rIndices[1] << 12) + rIndices[0];
	...
	/**
	 * Prüft, ob der Eintrag aus einer anderen Tabelle stammt. Wenn ja, ist
	 * getRetIndex( index, 0) der Index der Tabelle und ON_TABLE bestimmt, ob es aus
	 * onTables oder aus offTables kommt
	 */
	/**
	 * Checks if the entry comes from another table. If <code>true</code>,
	 * {@link #isOnTable(int) isOnTable(index)} returns if the source of the entry
