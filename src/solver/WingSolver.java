...
public class WingSolver extends AbstractSolver {
	...
	/**
	 * Wir suchen das Grid ab nach Zellen mit nur 2(3) Kandidaten xy (xyz). Wenn wir
	 * eine finden, versuchen wir eine weitere Zelle mit zwei Kandidaten im gleichen
	 * Block oder in der gleichen Spalte (im gleichen Block) zu finden, bei der ein
	 * Kandidat gleich, der zweite aber verschieden (beide gleich) ist - xz (xz).
	 * Wenn wir eine solche Zelle finden, suchen wir in der gleichen Zeile oder
	 * Spalte des ersten Elements (zweites Element in Spalte: nur Zeile) eine dritte
	 * Bivalue-Zelle, die den neuen Kandidaten mit der zweiten Zelle gemeinsam hat
	 * und deren zweiter Kandidat der fehlende Kandidat der ersten Zelle ist - yz
	 * (yz). Alle Zellen die die zweite und dritte Zelle (alle drei Zellen) sehen
	 * können, können z nicht als Kandidaten haben.
	 */
	/**
	 ...
	 * @return
	 */
	private SolutionStep createWWingStep(int cand1, int cand2, int index1, int index2, SudokuSet elimSet,
			boolean onlyOne) {
		globalStep.reset();
