...
public class UniquenessSolver extends AbstractSolver {
	...
	/**
	 * All already checked rectangles: One int aabbccdd with "aa"... the indices of
	 * the corners (sorted ascending)
	 */
	 ...
	 * is available or the sudoku has changed since the last call find a new one. If
	...
	/**
	 * - Feststellen, in welcher Unit1 sich die Zelle befindet - alle anderen Zellen
	 * dieser Unit1 anschauen, die im selben Block sind - wenn eine Zelle die selben
	 * zwei Kandidaten (plus beliebig viele andere) hat, für beide Zellen die Unit2
	 ...
	 * cells must not be givens.
	 */
	private SolutionStep findUniquenessForStartCell(int index11, boolean avoidable, SolutionType type,
			boolean onlyOne) {
...
//        System.out.println("allowedCand2: " + allowedCand2);
		...
		for (int i = 0; i < blockIndices.length; i++) {
			...
			if ((!avoidable && (sudoku.getValue(index12) == 0 && ((!allowMissing && (cell11 & cell12) == cell11)
				...
				for (int j = 0; j < unit11.length; j++) {
					...
					if ((!avoidable && (!allowMissing && (cell21 & cell11) == cell11 && (cell22 & cell11) == cell11))
							...
							|| (avoidable && ((sudoku.getValue(index21) == cand2 && !sudoku.isFixed(index21)
									&& sudoku.getValue(index22) == 0 && sudoku.isCandidate(index22, cand1)
									&& sudoku.getAnzCandidates(index22) == 2)
									|| (sudoku.getValue(index22) == cand1 && !sudoku.isFixed(index22)
											&& sudoku.getValue(index21) == 0 && sudoku.isCandidate(index21, cand2)
											&& sudoku.getAnzCandidates(index21) == 2)
									|| (sudoku.getValue(index21) == 0 && sudoku.isCandidate(index21, cand2)
											&& sudoku.getAnzCandidates(index21) == 2 && sudoku.getValue(index22) == 0
											&& sudoku.isCandidate(index22, cand1)
											&& sudoku.getAnzCandidates(index22) == 2)))) {
...
//        }
		...
		// Uniqueness Test 1: 3 cells have only 2 candidates -> delete cand1 and cand2
		// from the
		// fourth cell
		if (twoSize == 3) {
		...
		// Uniqueness Test 2/5/5A: one or two cells have 2 candidates, the other have
		// exactly
		// one additional candidate, which is the same for all cells: This candidate can
		// be deleted
		// from all cells that can see all UR cells with that candidate.
		// If there are only two additional candidate cells and if they are in the same
		// cell or row
		...
		// Uniqueness Test 3: If two UR cells have additional candidates, find in one of
		// their houses
		// additional (k - 1) cells, so that all cells contain exactly k candidates
		// (cand1 and cand2
		// may not appear in any of these cells). If we find such cells, the k
		// candidates can be
		...
		if (twoSize == 2) {
			...
			if (Sudoku2.getCol(i1) == Sudoku2.getCol(i2)) {
				step = checkUniqueness3(Sudoku2.COL, Sudoku2.COLS[Sudoku2.getCol(i1)], u3Cands, urMask, searchType,
						onlyOne);
				if (step != null && onlyOne) {
					// if steps should be cached that was already done
					return step;
			...
			if (Sudoku2.getBlock(i1) == Sudoku2.getBlock(i2)) {
				step = checkUniqueness3(Sudoku2.BLOCK, Sudoku2.BLOCKS[Sudoku2.getBlock(i1)], u3Cands, urMask,
						searchType, onlyOne);
				if (step != null && onlyOne) {
					// if steps should be cached that was already done
					return step;
...
//                System.out.println("   delCand: " + delCand);
		...
		// Uniqueness Test 6: Two cells with additional candidates located diagonally;
		// if in both lines and cols none of
		// the other candidates contain cand1 -> cand2 can be deleted from the diagonal
		// cells
		if (twoSize == 2) {
			int i1 = additionalCandidates.get(0);
			int i2 = additionalCandidates.get(1);
		...
		// they
		// have to be aligned diagonally) one of the cells with only two cands is the
		// corner; if
		// the line and the col through the other three cells contain only two
		// occurences
	...
	/**
	 * Avoidable Rectangle: If only one of cell21/cell22 is not set, cand1/cand2 can
	 * be deleted from that cell. If both cell21 and cell22 are not set, cell21 must
...
//            System.out.println("check AR " + index21 + "/" + index22);
	 ...
	 * @param cand2
	 */
	private SolutionStep checkCandForHiddenRectangle(int line, int col, int cand1, int cand2, SolutionType type,
			boolean onlyOne) {
...
//        System.out.println(tmpSet);
	 ...
	 * @return
	 */
	private SolutionStep checkUniqueness3(int unitType, int[] unit, short u3Cands, short urMask,
...
//            System.out.println("   " + u3Indices);
			SolutionStep step = checkUniqueness3Recursive(unitType, unit, u3Indices, u3Cands,
					new SudokuSet(additionalCandidates), 0, searchType, onlyOne);
			if (step != null && onlyOne) {
				return step;
			}
		}
	 ...
	 * @return
	 */
	private SolutionStep checkUniqueness3Recursive(int type, int[] unit, SudokuSet u3Indices, short candsIncluded,
			SudokuSet indicesIncluded, int startIndex, SolutionType searchType, boolean onlyOne) {
		SolutionStep step = null;
...
//                    System.out.println("  cells: " + aktIndices + "/" + isSameLineOrCol(aktIndices));
					// candidates can be deleted in all cells of the same unit, which are not
					// contained in
...
//                                            System.out.println("      ctd: " + unit1[j] + "/" + delCandsArray[k]);
	...
	private boolean checkRect(int i11, int i12, int i21, int i22) {
		...
		if (rectAnz < rectangles.length) {
			...
			Logger.getLogger(getClass().getName()).log(Level.WARNING,
					"Find Uniqueness: Kein Platz mehr in rectangles!");
		}
		// check it
		return true;
	}
	...
	public static void main(String[] args) {
		...
		// 342 844 348 149 949 854 362 368 892 894 896::");
		// sudoku.setSudoku(":0000:x:837159246.426837.96.9742.38.2..169..37..95.62.96.27...9..2683..28357469176.93182.:443
		// 449 453 469::");
		// sudoku.setSudoku(":0000:x:.78.....6.19.5.4.8.53...29.....76...761.4.9253..59.....2....7838.7.3.1..13.....4.:241
		// 441 344 844 866 471 671 694 695 596::");
		sudoku.setSudoku(
				":0000:x:..513.9.2..1..97..89..2.4..45.29.136962351847.1...6..9.349...78...8..39..89.736..:625 271 571 485::");
		sudoku.setSudoku(
