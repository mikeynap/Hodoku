...
public class BackgroundGeneratorThread implements Runnable {
	...
	/**
	 * Schedules a new creation run. If the thread is not yet running, it is
	 * started. The thread is signalled. Since it is possible, that the thread is
	 * still busy with another run, a flag is set as well.
	 */
	...
	/**
	 * The main thread: If it is signalled it checks, which type of puzzle is
	 * missing. As long as a missing puzzle type is found, the creation is
	 * continued.
	 */
	@Override
	public void run() {
			...
			try {
				...
				while (level == null && !thread.isInterrupted()) {
					...
					synchronized (this) {
						...
						puzzles1 = Options.getInstance().getPractisingPuzzles();
						if (DEBUG) {
							System.out.println("looking for pract: " + level + "/" + trLevel + "/"
									+ Options.getInstance().getActLevel() + "/"
									+ Options.getInstance().getPractisingPuzzlesLevel());
						}
						if (level == null && trLevel != -1 && Options.getInstance().getActLevel() >= trLevel) {
							for (int i = 0; i < puzzles1.length; i++) {
								if (puzzles1[i] == null) {
									...
									level = Options.getInstance()
											.getDifficultyLevel(Options.getInstance().getPractisingPuzzlesLevel());
					...
					if (DEBUG) {
						System.out.println(
								"  created in background: " + level.getName() + "/" + mode.name() + "/" + puzzle);
					}
					// and try again
					level = null;
					mode = null;
				...
				if (DEBUG) {
					System.out
							.println("Done (level = " + level + ", isInterrupted() = " + thread.isInterrupted() + ")!");
				}
	 ...
	 * should be created at all (if no trainig step is enabled, creation will be
